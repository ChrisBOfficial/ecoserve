                ...
                <div class="row">
                    <form class="form-inline">
                        ...
                        <div v-if="uploaded" class="form-group">
                            <button v-on:click="dataParse" type="submit" id="json-file" class="btn btn-primary">
                                Show JSON
                            </button>
                        </div>
                        <div v-if="uploaded" class="form-group">
                            <button v-on:click="emailParse" type="submit" id="email-btn" class="btn btn-primary">
                                Generate Email List
                            </button>
                        </div>
                    </form>
...
export default {
    ...
    methods: {
        ...
        dataFormat: function(results) {
            ...
            var data_obj = {
                familiarity: {
                    canada_thistle: { values: [] },
                    leafy_spurge: { values: [] },
                    musk_thistle: { values: [] },
                    plumeless_thistle: { values: [] },
                    sericea_lespedeza: { values: [] },
                    spotted_diffuse_knapweed: { values: [] },
                    russian_olive: { values: [] },
                    scotch_thistle: { values: [] },
                    eastern_redcedar: { values: [] },
                    smooth_brome: { values: [] }
                },
            ...
            for (var i = 3; i < data.length; i++) {
                ...
                for (var j = 0; j < l1.length; j++) {
                    for (var k = 0; k < l2.length; k++) {
                        ...
                        data_obj[l1[j]][l2[k]].sum = data_obj[l1[j]][l2[k]].values
                            .filter(Boolean)
        ...
        chartData: function(inc_data) {
            ...
            svg.attr("height", height * offset - 30).attr("width", width * 2 + 80);
        },
